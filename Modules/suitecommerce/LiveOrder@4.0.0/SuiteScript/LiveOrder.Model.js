	...
	)
{
	...
	,	submit: function submit ()
		{
			//nlapiLogExecution('DEBUG', 'got here', 'got here');
			...
			// if(searchresults){
			// 	var customFields = {};
			// 	customFields['custbody_hp_contact'] = searchresults[0].getValue('entityid');
			// 	customFields['custbody_hp_fax_number'] = searchresults[0].getValue('fax');
			// 	customFields['custbody_hp_phone_number'] = searchresults[0].getValue('phone');
	...
	,	setTransactionBodyField: function setTransactionBodyField (data)
		{
			...
			if (this.isSecure && !_.isEmpty(data.options) &&ModelsInit.session.isLoggedIn2())
			{
				...
				nlapiLogExecution('DEBUG','test',JSON.stringify(data.options));
