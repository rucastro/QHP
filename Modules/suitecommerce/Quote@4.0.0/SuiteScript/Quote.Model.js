	...
	)
{
	...
	,	preSubmitRecord: function() {
			//set location
			this.record.setFieldValue('location', 10);

			var itemCount = parseInt(this.record.getLineItemCount('item'));

			for (var i = 1; i <= itemCount; i++) {
				this.record.setLineItemValue('item','location', i, 10);
			}

			var profile = ProfileModel.get();
			var filters = new Array();
			filters[0] = new nlobjSearchFilter('email', null, 'is', profile.email);
			filters[1] = new nlobjSearchFilter('company', null, 'is', profile.internalid);

			var searchresults = nlapiSearchRecord('contact', 'customsearch_webstore_contact_search', filters, null);

			if(searchresults){
				this.record.setFieldValue('custbody_hp_contact', searchresults[0].getValue('entityid'));
				this.record.setFieldValue('custbody_hp_fax_number', searchresults[0].getValue('fax'));
				this.record.setFieldValue('custbody_hp_phone_number', searchresults[0].getValue('phone'));
			}	
		}
